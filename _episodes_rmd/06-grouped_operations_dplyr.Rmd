...
gapminder_clean %>% 
  ...
  # select a few columns only for visibility purposes only
...
Need an example where things can go wrong...

```{r}
gapminder_clean %>% 
  group_by(year) %>% 
  filter(income_per_person == min(income_per_person, na.rm = TRUE)) %>% 
  # this still counts per year (because did not ungroup)
  count(country)
```
...
This is a kind of interesting insight: relatively speaking, all countries actually 
change similarly.
...
Grouped operations can be used to ask many questions...
...
Lead and lag operations (e.g. rate of change)
...
- then do `group_by() %>% mutate()`
  ...
  - another is to do vector functions like `lead()`, `lag()`, `cumsum`, etc. See dplyr cheatsheet. 

Idea for exercise:

- plot number of missing values per year for some variable (to illustrate use `sum(is.na(x))`).
